...
public class medicineReportProvider extends ContentProvider {
...
//        path : authority 중에서 다음 path에 해당하며 path/# 으로 하는 경우 path 다음에 모든 숫자를 지칭하며 path/* 의 경우 숫자와 문자를 포함한다.
        ...
        sUriMatcher.addURI(medicineReportContract.CONTENT_AUTHORITY, medicineReportContract.PATH_VEHICLE, REMINDER);

        // content://com.delaroystudios.alarmreminde/reminder-path/101
        sUriMatcher.addURI(medicineReportContract.CONTENT_AUTHORITY, medicineReportContract.PATH_VEHICLE + "/#", REMINDER_ID);
    ...
    String sortOrder) {
        ...
        switch (match) {
            case REMINDER:
                cursor = database.query(medicineReportContract.medicineReportEntry.TABLE_NAME, projection, selection, selectionArgs, null, null, sortOrder);
                break;
            case REMINDER_ID:
                selection = medicineReportContract.medicineReportEntry._ID + "=?";
    ...
    public String getType(@NonNull Uri uri) {
        ...
        switch (match) {
            case REMINDER:
                return medicineReportContract.medicineReportEntry.CONTENT_LIST_TYPE;
            case REMINDER_ID:
                return medicineReportContract.medicineReportEntry.CONTENT_ITEM_TYPE;
            default:
    ...
    private Uri insertReminder(Uri uri, ContentValues values) {
        ...
        long id = database.insert(medicineReportContract.medicineReportEntry.TABLE_NAME, null, values);
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case REMINDER:
                rowsDeleted = database.delete(medicineReportContract.medicineReportEntry.TABLE_NAME, selection, selectionArgs);
                break;
            case REMINDER_ID:
                selection = medicineReportContract.medicineReportEntry._ID + "=?";
                selectionArgs = new String[] { String.valueOf(ContentUris.parseId(uri)) };
    ...
    private int updateReminder(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        int rowsUpdated = database.update(medicineReportContract.medicineReportEntry.TABLE_NAME, values, selection, selectionArgs);
